...
export default function QuickAddModal({ isOpen, onClose }) {
  ...
  const handleAction = (id) => {
    if (['food', 'barcode', 'scan', 'voice'].includes(id)) setFoodDialogOpen(true);
    else if (id === 'water')    setWaterDialogOpen(true);
    else if (id === 'weight')   setWeightDialogOpen(true);
    else if (id === 'exercise') setExerciseDialogOpen(true);
    onClose();
  };
  ...
  return (
    <>
      ...
      <LogValueDialog isOpen={weightDialogOpen} onClose={() => setWeightDialogOpen(false)} title="Log Weight"   unit="kg"      value={todayLog.weight           || 70} step={0.1} min={30} max={300} color="#10b981" onSave={(v) => upsertLog({ weight: v })} />
